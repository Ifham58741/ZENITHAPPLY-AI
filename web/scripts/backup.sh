#!/bin/bash
# Database backup and recovery script for ZENITHAPPLYAI application

set -e

# Configuration
BACKUP_DIR="/var/backups/ZENITHAPPLYAI"
POSTGRES_CONTAINER="ZENITHAPPLYAI_db_1"
POSTGRES_USER="postgres"
POSTGRES_DB="ZENITHAPPLYAI"
RETENTION_DAYS=7
S3_BUCKET="ZENITHAPPLYAI-backups"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
BACKUP_FILE="ZENITHAPPLYAI_backup_${TIMESTAMP}.sql.gz"
LOG_FILE="/var/log/ZENITHAPPLYAI/backup.log"
...
cleanup_old_backups() {
    ...
    # Local cleanup
    find "$BACKUP_DIR" -name "ZENITHAPPLYAI_backup_*.sql.gz" -type f -mtime +$RETENTION_DAYS -delete
    ...
    if command -v aws &> /dev/null; then
        ...
        aws s3 ls "s3://$S3_BUCKET/" | grep "ZENITHAPPLYAI_backup_" | while read -r line; do
...
list_backups() {
    ...
    find "$BACKUP_DIR" -name "ZENITHAPPLYAI_backup_*.sql.gz" -type f | sort
    
    if command -v aws &> /dev/null; then
        log "Available S3 backups:"
        aws s3 ls "s3://$S3_BUCKET/" | grep "ZENITHAPPLYAI_backup_"
    fi
}
